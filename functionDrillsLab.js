...
function add(pamOne, pamTwo){
  return(Number(pamOne)+ Number(pamTwo));
}
...
console.log(exclaimThree('Exclaim Three'))
...
function nameCheck(name){
  if(name != `Bryan` && name != `Steven`){return `Cool name, ${name}`}
  return(name === "Steven" ? "What is up Steven?": "Hey Bryan!")
}
...
function printAllNames(someArray){
...
const bestMovie = oneParameter => {return oneParameter.toUpperCase()}
...
//long way creating new list
function theEliminator(x,y){
  newContestants = x
  for(i=0; i<newContestants.length; i++){
    if(newContestants[i] === loser){
      newContestants.splice(i,1)
      i--
    }
    }
...
let newList = theEliminator(contestants,loser)
console.log(newList)
...
function emailCheck(email){
  return(String(email).trim().includes("@") ? `email verified` : `must provide a valid email address`)

}

console.log(emailCheck(" somerandome@email.c  om    "))
...
const frogBuy = (items) => items/3

let totalFrogs = frogBuy(5)
...
const frogBuy2 = (items) => items/3 >> 0
//function using Math
// const frogBuy2 = (items) => {return Math.floor(items/3) }

let totalFrogs2 = frogBuy2(5)
...
let arrayIsAscending= newFuncOne(sampleArray)
